...
from single_instrument.utils import exists, default
from single_instrument.music_diti import DiTi
...
class UNet1d_new(nn.Module):
    def __init__(
        ...
        self.use_context_features = use_context_features
        self.use_context_channels = use_context_channels
        self.extract_upsample = extract_upsample
        ...
        self.upsamples = nn.ModuleList(
            [
                UpsampleBlock1d(
                    ...
                    attention_features=attention_features,
                    attention_multiplier=attention_multiplier,
                    extract_channels=context_channels[i + 1] if self.extract_upsample else 0,
                )
                for i in reversed(range(num_layers))
            ]
        )
        ...
        self.to_out = Unpatcher(
            in_channels=channels,
            out_channels=out_channels if not self.extract_upsample else out_channels + context_channels[0],
            blocks=patch_blocks,
            factor=patch_factor,
            context_mapping_features=context_mapping_features,
        )
    ...
    def forward(
        ...
        out_context: List[Tensor] = []

        for i, upsample in enumerate(self.upsamples):
            skips = skips_list.pop()
            if not self.extract_upsample:
                x = upsample(x, skips, mapping=mapping, embedding=embedding)
            else:
                x, extract = upsample(x, skips, mapping=mapping, embedding=embedding)
                out_context.append(extract)

        x = x + skips_list.pop()
        x = self.to_out(x, mapping)

        if self.extract_upsample:
            x, context0 = torch.split(x, [self.out_channels, self.context_channels[0]], dim=1)
            out_context.append(context0)

        if self.extract_upsample:
            return x, out_context[::-1]
        else:
            return x
...
class Model(nn.Module):
    def __init__(self,
                 net,
                 diffusion_sigma_distribution=audio_diffusion_pytorch.LogNormalDistribution(mean=-3.0, std=1.0),
...
class RNNDevil(nn.Module):
    def __init__(self, patch_size, n, diff_model, device):
        super().__init__()
    ...
    def cell(self, input_patch, hidden_layer, sigmas):
        diff_output = self.diff_model.unet(input_patch, sigmas)
        compressed = torch.cat([diff_output, hidden_layer], dim=1)
        model_out = self.model(compressed, sigmas)
        return model_out[:, :self.n, :], model_out[:, self.n:, :]

    def forward(self, input_patches, sigmas):
        b = input_patches.shape[0]
        n = input_patches.shape[1]
        l = input_patches.shape[2]
        m = l // self.patch_size
        hidden_layer = torch.zeros((b, 4 * n, self.patch_size)).to(self.device)
        out = torch.zeros((b, n, l)).to(self.device)
        for i in range(m):
            diff_fixed, hidden_layer = self.cell(input_patches[:, :, i * self.patch_size: (i + 1) * self.patch_size],
                                                 hidden_layer, sigmas)
            out[:, :, i * self.patch_size: (i + 1) * self.patch_size] = diff_fixed
        return out


class RNNUnetDevil(nn.Module):
    def __init__(self, patch_size, n, diff_model, device):
        super().__init__()
        self.diff_model = diff_model
        self.diff_model.requires_grad_(False)
        self.hidden_channels = [16, 16, 32, 64, 64]
        self.hidden_lengths = [2 ** 18, 2 ** 16, 2 ** 14, 2 ** 12, 2 ** 10]
        self.model = UNet1d_new(
            in_channels=n,
            channels=64,
            patch_factor=4,
            patch_blocks=1,
            ...
            use_context_time=True,
            out_channels=n,
            context_channels=self.hidden_channels,
            extract_upsample=True,
        ).to(device)
        self.n = n
        self.patch_size = patch_size
        self.device = device

    def cell(self, input_patch, hiddens, sigmas):
        diff_output = self.diff_model.unet(input_patch, sigmas)
        model_out, new_hiddens = self.model(diff_output, sigmas, channels_list=hiddens)
        return model_out, new_hiddens
    ...
    def forward(self, input_patches, sigmas):
        ...
        l = input_patches.shape[2]
        m = l // self.patch_size
        hiddens = [torch.zeros((b, ch_cnt, L)).to(self.device)
                   for ch_cnt, L in zip(self.hidden_channels, self.hidden_lengths)]
        out = torch.zeros((b, n, l)).to(self.device)
        for i in range(m):
            diff_fixed, hiddens = self.cell(input_patches[:, :, i * self.patch_size: (i + 1) * self.patch_size],
                                            hiddens, sigmas)
            out[:, :, i * self.patch_size: (i + 1) * self.patch_size] = diff_fixed
        return out


class UNetDevil(nn.Module):
    def __init__(self, patch_size, n, diff_model, device):
        super().__init__()
        self.diff_model = diff_model
    ...
    def forward(self, input_patches, sigmas):
        ...
        for i in range(m):
            diff_out.append(
                self.diff_model.unet(input_patches[:, :, i * self.patch_size: (i + 1) * self.patch_size], sigmas))

        out = self.model(torch.cat(diff_out, dim=1), sigmas)
        return out.reshape(b, self.n, l)


class RNNOverlapDevil(nn.Module):
    def __init__(self, diff_model, device, n=4, patch_size=262144, overlap_size=262144 // 4):
        super().__init__()
        self.diff_model = diff_model
        self.diff_model.requires_grad_(False)
        self.patch_size = patch_size
    ...
    def cell(self, input_patch, hidden_layer, sigmas):
        ...
        diff_output = self.diff_model.unet(input_patch, sigmas)
        concat = torch.cat((diff_output, hidden_layer), dim=1)
        model_out = self.model(concat, sigmas)
        return model_out[:, :n, :], model_out[:, n:, :]

    def weight_cell(self, left, right, weight_hidden, sigmas):
        n = left.shape[1]
        concat = torch.cat((left, right, weight_hidden), dim=1)
        model_out = self.weight_model(concat, sigmas)
        return torch.sigmoid(model_out[:, :n, :]), model_out[:, n:, :]
    ...
    def forward(self, input_patches, sigmas):
        ...
        hidden_layer = torch.zeros((b, 4 * n, self.patch_size)).to(self.device)
        out = torch.zeros((b, n, l)).to(self.device)
        m = (l - self.patch_size) // (self.patch_size - self.overlap_size) + 1
        rnn_outs = []
        for i in range(m):
            start = i * (self.patch_size - self.overlap_size)
            end = start + self.patch_size
            diff_fixed, hidden_layer = self.cell(input_patches[:, :, start: end], hidden_layer, sigmas)
            pos = i * (self.patch_size - self.overlap_size)
            shift_l = (self.overlap_size if i > 0 else 0)
            shift_r = self.patch_size + (-self.overlap_size if i < m - 1 else 0)
            out[:, :, pos + shift_l: pos + shift_r] += diff_fixed[:, :, shift_l: shift_r]
            rnn_outs.append(diff_fixed)

        weight_hidden = torch.zeros((b, 4 * n, self.patch_size)).to(self.device)
        for i in range(len(rnn_outs) - 1):
            left = rnn_outs[i]
            right = rnn_outs[i + 1]
            weight, weight_hidden = self.weight_cell(left, right, weight_hidden, sigmas)
            weight = weight[:, :, :self.overlap_size]
            pos = (i + 1) * (self.patch_size - self.overlap_size)
            out[:, :, pos: pos + self.overlap_size] = left[:, :, -self.overlap_size:] * weight + right[:, :,
                                                                                                 :self.overlap_size] * (
                                                                  1 - weight)
...
class UNetOverlapDevil(nn.Module):
    def __init__(self, diff_model, device, n=4, patch_size=131072, overlap_size=131072 // 4):
        super().__init__()
        self.diff_model = diff_model
        self.diff_model.requires_grad_(False)
        self.patch_size = patch_size
    ...
    def forward(self, input_patches, sigmas):
        ...
        n = input_patches.shape[1]
        l = input_patches.shape[2]
        m = (5 if l // self.patch_size == 4 else 13)
        diff_out = []
        for i in range(m):
            start = i * (self.patch_size - self.overlap_size)
            end = start + self.patch_size
            diff_out.append(self.diff_model.unet(input_patches[:, :, start: end], sigmas))
        diff_out = self.model(torch.cat(diff_out, dim=1), sigmas)
        diff_out = diff_out.reshape(b, m, n, self.patch_size)

        w_schedule = torch.from_numpy(np.linspace(1.0, 0.0, self.overlap_size)).to(self.device)
        out = torch.zeros((b, n, l)).to(self.device)
        for i in range(m):
            diff_fixed = diff_out[:, i, :, :]
            pos = i * (self.patch_size - self.overlap_size)
            shiftl = (self.overlap_size if i > 0 else 0)
            shiftr = self.patch_size + (-self.overlap_size if i < m - 1 else 0)
            out[:, :, pos + shiftl: pos + shiftr] += diff_fixed[:, :, shiftl: shiftr]
            if shiftl > 0:
                out[:, :, pos: pos + shiftl] += diff_fixed[:, :, :shiftl] * (1 - w_schedule)
            if shiftr < self.patch_size:
                out[:, :, pos + shiftr: pos + self.patch_size] += diff_fixed[:, :, shiftr:] * w_schedule
        return out


class MultiDevil(nn.Module):
    def __init__(self, diff_model, patch_size, overlap_size, device, n_layers=2, widen=2, h_dim=128, emb_dim=128):
        super().__init__()
        self.diff_model = diff_model
        self.diff_model.requires_grad_(False)
        self.patch_size = patch_size
        ...
        self.widen = widen
        self.initial_layer = nn.Linear(patch_size * 2, self.h_dim)
        ...
        self.final_layer = nn.Linear(self.h_dim, overlap_size)

    def forward(self, x_t, sigmas):
        left = self.diff_model.unet(x_t[:, :, :self.patch_size], sigmas).to(x_t.device).squeeze(1)
        right = self.diff_model.unet(x_t[:, :, -self.patch_size:], sigmas).to(x_t.device).squeeze(1)
        emb = self.embed(sigmas)

        cur = self.initial_layer(torch.cat([left, right], dim=1))
        ...
        w = self.final_layer(cur)
        middle = ((left[:, -self.overlap_size:] + right[:, :self.overlap_size]) / 2.0 - w).unsqueeze(1)
        return torch.cat((left[:, :-self.overlap_size].unsqueeze(1),
                          middle,
                          right[:, self.overlap_size:].unsqueeze(1)), dim=2)


class MultiUnetDevil(nn.Module):
    def __init__(self, diff_model, patch_size, overlap_size, device):
        super().__init__()
        self.diff_model = diff_model
        self.diff_model.requires_grad_(False)
        self.patch_size = patch_size
    ...
    def forward(self, x_t, sigmas):
        left = self.diff_model.unet(x_t[:, :, :self.patch_size], sigmas).to(x_t.device)
        right = self.diff_model.unet(x_t[:, :, -self.patch_size:], sigmas).to(x_t.device)
        out = self.model(torch.cat([left, right], dim=1), sigmas)
        middle = (left[:, :, -self.overlap_size:] + right[:, :, :self.overlap_size]) / 2 - out[:, :, :self.overlap_size]
        return torch.cat((left[:, :, :-self.overlap_size],
                          middle,
                          right[:, :, self.overlap_size:]), dim=2)


class TransformerDevil(nn.Module):
    def __init__(self, diff_model, base_size, stride, device):
        super().__init__()
        self.diff_model = diff_model
        self.diff_model.requires_grad_(False)
        self.diti = DiTi(base_size=(1, base_size),
                         patch_size=(1, 128),
                         in_channels=4,
                         hidden_size=768,
                         depth=6,
                         ...
                         stride=(0, stride),
                         add_pos_emb=False,
                         use_rotary_attn=True).to(device)
        self.base_size = base_size
        self.stride = stride
        self.device = device

    def forward(self, x, t):
        MW = (x.shape[2] - self.base_size) // self.stride + 1
        scores = []
        for i in range(MW):
            L = i * self.stride
            R = L + self.base_size
            scores.append(torch.unsqueeze(self.diff_model.unet(x[:, :, L:R], t), dim=2))
        diti_input = torch.stack(scores, dim=1)
        N, _, C, H, W = diti_input.shape
        diti_input = diti_input.reshape((N, 1, MW, C, H, W))
        return torch.squeeze(self.diti(diti_input, t, (0, 0)), dim=2)
