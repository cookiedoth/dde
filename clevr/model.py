...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.label_emb_x = GaussianFourierProjection(embed_dim=time_embed_dim // 2)
        self.label_emb_y = GaussianFourierProjection(embed_dim=time_embed_dim // 2)
        # unconditional label
    ...
    def forward(self, x, t, y, masks):
        ...
        label_emb[masks] = torch.cat((self.label_emb_x(y[masks][:, 0]), self.label_emb_y(y[masks][:, 1])), dim=1)
...
class Model(nn.Module):
    def __init__(self,
                 net,
                 diffusion_sigma_distribution=audio_diffusion_pytorch.LogNormalDistribution(mean=-1.0, std=1.6),
    ...
    def denoise_fn(self, x_noisy, sigmas, y, mask):
        c_skip, c_out, c_in, c_noise = self.get_scale_weights(sigmas, x_noisy)
        x_pred = self.unet(c_in * x_noisy, c_noise, y, mask)
    ...
    def forward(self, x_noisy, sigmas, y, mask):
        x_denoised = self.denoise_fn(x_noisy, sigmas, y, mask)
        sigmas_padded = pad_dimensions(sigmas, x_noisy)
